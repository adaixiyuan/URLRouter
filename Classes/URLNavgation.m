...
@implementation URLNavgation

URLSingletonM(URLNavgation)
...
+ (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated replace:(BOOL)replace
{
    if (!viewController) {
        ...
        if([viewController isKindOfClass:[UINavigationController class]]) {
            [URLNavgation setRootViewController:viewController];
        } // 如果是导航控制器直接设置为根控制器
        else {
            UINavigationController *navigationController = [URLNavgation sharedURLNavgation].currentNavigationViewController;
            if (navigationController) { // 导航控制器存在
                ...
                [URLNavgation sharedURLNavgation].applicationDelegate.window.rootViewController = navigationController;
...
+ (void)presentViewController:(UIViewController *)viewController animated: (BOOL)flag completion:(void (^ __nullable)(void))completion
{
    if (!viewController) {
        ...
        UIViewController *currentViewController = [[URLNavgation sharedURLNavgation] currentViewController];
        if (currentViewController) { // 当前控制器存在
            ...
            [URLNavgation sharedURLNavgation].applicationDelegate.window.rootViewController = viewController;
        }
    }
}
...
+ (void)popTwiceViewControllerAnimated:(BOOL)animated {
    [URLNavgation popViewControllerWithTimes:2 animated:YES];
}
...
+ (void)popViewController:(UIViewController *)viewController animated:(BOOL)animated{
    UIViewController *currentViewController = [[URLNavgation sharedURLNavgation] currentViewController];
    [currentViewController.navigationController popToViewController:viewController animated:YES];
}
+ (void)popToRootViewControllerAnimated:(BOOL)animated {
    UIViewController *currentViewController = [[URLNavgation sharedURLNavgation] currentViewController];
    NSUInteger count = currentViewController.navigationController.viewControllers.count;
    [URLNavgation popViewControllerWithTimes:count-1 animated:YES];
}
...
+ (void)dismissViewControllerWithTimes:(NSUInteger)times animated: (BOOL)flag completion: (void (^ __nullable)(void))completion {
    UIViewController *rootVC = [[URLNavgation sharedURLNavgation] currentViewController];
